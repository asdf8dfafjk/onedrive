...
*   Nix/NixOS, Package is available on stable since since 20.03. Use package `onedrive` either by adding it to `configuration.nix` or by using the command `nix-env -iA <channel name>.onedrive`. For a service (currently only on unstable channel or stable since 20.09, use `services.onedrive.enable=true` in your `configuration.nix`. This would also install a package. Use `services.onedrive.package=...` for a custom package (recommended since generally onedrive in repos lags upstream).
